...
| US-02 | Candidate | As a candidate, I want to maintain my profile and CV so employers can evaluate me. | Candidate can upload a validated CV once, reuse it automatically when applying, and retain the exact submitted CV on each application. | `routes/candidateRoutes.js`, `controllers/candidateController.js`, `controllers/applicationController.js`, `middleware/upload.js` |
...
| US-10 | Candidate/Employer | As an application participant, I want a lifecycle timeline so I know the current stage and how the application reached it. | Initial submission and every authorized status transition are recorded; both the candidate and owning employer can view the timeline. | `models/applicationModel.js`, `public/js/candidate-dashboard.js`, `public/js/company-dashboard.js`, `routes/applicationRoutes.js` |
| US-11 | Candidate | As a candidate, I want to opt into automatic applications so suitable new jobs are not missed. | Feature is off by default, requires a CV, lets the candidate choose a threshold, avoids duplicates, and marks every generated application and lifecycle event as automatic. | `services/autoApplyService.js`, `controllers/candidateController.js`, `controllers/jobController.js`, `public/js/candidate-dashboard.js` |
